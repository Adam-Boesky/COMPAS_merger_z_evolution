...
def find_metallicity_distribution(redshifts, min_logZ_COMPAS, max_logZ_COMPAS, min_logZ=-12.0, max_logZ=0.0, step_logZ=0.01):
    ...
    mssfr.logNormalPrescription = None
    mssfr.GSMFprescription      = 'Panter et al. (2004) Single'
    mssfr.ZMprescription        = 'Langer et al. +offset (2006)'

    pdf = mssfr.returnFractionMZ_GSMF(min(metallicities), max(metallicities), redshifts)
    pdf = np.divide(pdf, max(metallicities) - min(metallicities))
    print(pdf)

    # norm = pdf.sum(axis=-1) * step_logZ
    # dPdlogZ = pdf / norm[:,np.newaxis]
    dZdlogZ = np.diff(np.append(metallicities, np.exp(max_logZ + step_logZ))) / step_logZ
    dPdlogZ = pdf * dZdlogZ
    dPdlogZ = dPdlogZ[:,np.newaxis]
...
def find_detection_rate(path, dco_type="BBH", lognormal=False, Zprescription='MZ_GSMF', SFRprescription='Madau et al. (2017)', GSMFprescription='Panter et al. (2004) Single', 
                        ZMprescription='Langer et al. +offset (2006)', logNormalPrescription=None, merger_output_filename=None, weight_column=None,
